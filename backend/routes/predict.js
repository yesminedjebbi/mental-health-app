...
router.post('/', (req, res) => {
  console.log('Requête reçue pour la prédiction');  

  const { depression, anxiety, happiness, loneliness, anger, activity_progress, time_spent } = req.body;
  console.log('Données reçues:', { depression, anxiety, happiness, loneliness, anger, activity_progress, time_spent }); 
  ...
  pythonProcess.stdout.on('data', (data) => {
    console.log('Données du script Python:', data.toString());  
    if (!responseSent) {
      try {
        ...
        console.error('Erreur lors de la conversion des données du script Python:', error);  
  ...
  pythonProcess.on('error', (err) => {
    console.error(`Erreur de lancement du script Python: ${err}`); 
    if (!responseSent) {
