...
const InterestSelectionScreen = ({ route,navigation }) => {
  ...
  const saveInterests = async () => {
    try {
      const response = await fetch("http://192.168.1.6:5000/api/interests/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId:userId, interests: selectedInterests }),
      });
